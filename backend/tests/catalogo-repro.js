// tests/catalogo-repro.js — REGRESSÃO DE PAR CASADO.
// Roda o Validador no SPED do POSTO CG e confere contagens contra o relatório catálogo de referência
// "analise posto campos.pdf" (39778541000180, 06/2026). Cada número foi verificado byte-a-byte.
// Uso: node tests/catalogo-repro.js   (skip se a fixture de cliente não estiver presente)
...
if (!fs.existsSync(FIX)) { console.log('SKIP catalogo-repro: fixture ausente (' + FIX + ')'); process.exit(0); }
...
const EXPECT = [
    ...
    ['DOC-C100-VLDOC-01', 'gte', 200], // catálogo de referência corta em 200; motor acha ~742
    ['DOC-C170-ICMSSEMBASE-01', 'eq', 1],
    ['DOC-C190-ICMSSEMBASE-01', 'eq', 1],
    ['DOC-C190-REDBC-01', 'eq', 1],
    ['EST-9900-REGBLC-01', 'eq', 23],
    // DIVERGE de PROPÓSITO do catálogo de referência (que acha 12): a fórmula correta (Σ VL_ICMS dos C170) mostra
...
console.log(`catalogo-repro: ${pass} ok, ${fail} falhas (regras disparadas: ${Object.keys(byId).length})`);
