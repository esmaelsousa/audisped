// COMB-1310-ANP-01 (E-Auditoria 2033) — Variação entre estoque FÍSICO (fechamento) e ESCRITURAL
// de cada tanque (1310) acima do limite ANP de 0,6% (art. 5º da Portaria DNC nº 26, de 13.11.1992).
// É ALERTA (ADV): não é rejeição do PVA — o E-Auditor o trata como "indício para início de possível
// ação fiscal" (perda/ganho de produto além da volatilidade tolerada, ou físico não medido).
//
// Fórmula do E-Auditor (reproduzida byte-a-byte): percentual = |ESTQ_ESCR − FECH_FISICO| / ESTQ_ESCR.
...
// Ref. hierárquica exibida pelo E-Auditor: COD_ITEM + DT do 1300 pai imediato.
//
// Verificado no par casado arq 2028 (S CRUZ / AUTO POSTO DA CHAPADA 09069475000109, 01/2026):
// E-Auditor 2033 = 146 ocorrências (109 com físico=0 → 100%; o restante variação real de 0,7%–403%).
// ⚠️ Cross-empresa: ERPs que não medem o físico deixam FECH_FISICO=0 → 100% em todos os tanques;
// é o próprio indício que o E-Auditor aponta. Como é ADV, nunca bloqueia — só orienta.
...
module.exports = {
    ...
    detectar(model) {
        ...
        for (const l of model.linhas) {
            ...
            // O E-Auditor arredonda o percentual para 1 casa decimal (a precisão do "0,6%" da norma
            ...
            if (pctArred > 0.6) {
                ...
                // variação volumétrica real — o E-Auditor mistura as duas sob o mesmo código; nós rotulamos.
